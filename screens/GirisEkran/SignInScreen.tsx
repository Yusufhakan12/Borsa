    ...
    <View style={theme === "light" ? styles.root : SignInScreenDark.root}>
      ...
      {error ? (
        mail.length <= 0 ? (
          <Text style={styles.error}>
            {langauge.dil === false
              ? SIGNINTRANSLATION[3].Turkce
              : SIGNINTRANSLATION[3].English}
          </Text>
        ) : null
      ) : null}
      <CustonInput
      ...
      {error ? (
        password.length <= 0 ? (
          <Text style={styles.error}>
            {" "}
            {langauge.dil === false
              ? SIGNINTRANSLATION[2].Turkce
              : SIGNINTRANSLATION[2].English}
          </Text>
        ) : null
      ) : null}
      {invalidError ? <Text style={styles.error}>Invalid E-mail</Text> : null}
      <CustomButton
        onpress={onSignInPressed}
        text={
          langauge.dil === false
            ? SIGNINTRANSLATION[0].Turkce
            : SIGNINTRANSLATION[0].English
        }
      />
      <Pressable onPress={onCreateAccount}>
        <Text style={theme === "light" ? styles.text : SignInScreenDark.text}>
          {langauge.dil === false
            ? SIGNINTRANSLATION[1].Turkce
            : SIGNINTRANSLATION[1].English}
        </Text>
      </Pressable>
      ...
      <View style={{marginTop: 590, position: "absolute"}}>
        ...
        >
          ...
          >
            <Text style={{color: "black", fontSize: 12, fontWeight: "800"}}>
              English
            </Text>
          </TouchableOpacity>
        </View>
